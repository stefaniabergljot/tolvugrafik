...
//
// Pendulum
// Að miklu leyti byggt á robotArm úr síðustu viku
//
//
// Stefanía Bergljót
//////////////////
...
// Parameters controlling the size of the pendulum
...
function updateAngles() {
	for (var i = 0; i < 2; i++) {
		theta[i] += thetaInc[i];
		if (Math.abs(theta[i]) > maxTheta[i]) {
			thetaInc[i] *= -1; 
		}
	}
}
...
var render = function() {
	...
	modelViewMatrix = mult(mv, translate(0.0, UPPER_ARM_HEIGHT, 0.0));
	modelViewMatrix = mult(modelViewMatrix, rotateZ( theta[UpperArm] ));
	upperArm();
	
	modelViewMatrix = mult(modelViewMatrix, translate(0, -1.0*LOWER_ARM_HEIGHT, 0.0));
