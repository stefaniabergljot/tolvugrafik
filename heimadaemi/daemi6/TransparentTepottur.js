...
var colorCutoff = 1.8;
var sizeChangeInc = 0.01;
var rate = 0.01;
var rateIncrement = 0.001;
var theta = 0;
...
window.onload = function init() {
	...
	gl.uniform1f(gl.getUniformLocation(program, "colorCutoff"), colorCutoff);
    ...
    // Event listener for mousewheel
	 ...
	 window.addEventListener("keydown", function(e) {
		 switch(e.keyCode) {
			 case 37: 
				colorCutoff += 0.01;
				if (colorCutoff > 3.5) colorCutoff = 3.5;
				gl.uniform1f(gl.getUniformLocation(program, "colorCutoff"), colorCutoff);
				break;
			case 39:
				colorCutoff -= 0.01;
				if (colorCutoff < 0.5) colorCutoff = 0.5;
				gl.uniform1f(gl.getUniformLocation(program, "colorCutoff"), colorCutoff);
				break;
			case 38:
				rate += rateIncrement;
...
function render() {
	...
	var sz = (Math.sin(theta)+1)*0.5;
	

    gl.clear( gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);

    modelViewMatrix = lookAt( vec3(0.0, 0.0, zDist), at, up );
	modelViewMatrix = mult(modelViewMatrix, scalem(sz, sz, sz));
